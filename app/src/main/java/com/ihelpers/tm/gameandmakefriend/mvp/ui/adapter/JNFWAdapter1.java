...
public class JNFWAdapter1 extends CommonAdapter<Item> {
    public JNFWAdapter1(Context context, int layoutId, List<Item> datas) {
        super(context, layoutId, datas);
    }

    @Override
    protected void convertView(ViewHolder holder, Item item, int position) {
        holder.setImageResource(R.id.item_image_ig,item.getIcon());
    }
}
