...
import com.ihelpers.tm.gameandmakefriend.listner.OnSideBarTouchListener;
...
public class SideBar extends View {
    ...
    private int INDEX_COLOR = Color.parseColor("#FF6600");//索引字符颜色
    ...
    //索引字符数组
    public String[] indexArray = { "A", "B", "C", "D", "F", "G", "H", "J",
    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0; i < indexArray.length; i++) {
            ...
            float x = (mWidth - SizeUtils.getTextWidth(mTextPaint, index)) / 2;
            float y = mMarginTop + mHeight * i + (mHeight + SizeUtils.getTextHeight(mTextPaint, index)) / 2;
            canvas.drawText(index.toUpperCase(), x, y, mTextPaint);
        }
    }
    ...
    private void getMaxTextSize() {
        for (String index : indexArray) {
            ...
            maxHeight = Math.max(maxHeight, SizeUtils.getTextHeight(mTextPaint, index));
        }
    }
